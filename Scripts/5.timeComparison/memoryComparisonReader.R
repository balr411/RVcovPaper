...
for(iter in 1:5){
  for(i in 1:nrow(genes)){
    ...
    gene <- genes[[1]][i]
    time_file_twoStage <- str_glue("memoryComparison/timeOutputs/twoStage/{iter}/c2/{gene}/time.txt")
    ...
    time_file_trad <- str_glue("memoryComparison/timeOutputs/trad/{iter}/c2/{gene}/time.txt")
