...
for(iter in iters){
  print(iter)
  dir_to_make <- str_glue("memoryComparison/twoStageCovariance/{iter}/9999/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/twoStageRaremetal/{iter}/9999/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/fullCovariance/{iter}/9999/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/fullRaremetal/{iter}/9999/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/timeOutputs/twoStage/{iter}/9999/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/timeOutputs/trad/{iter}/9999/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
}
...
for(iter in iters){
  print(iter)
  dir_to_make <- str_glue("memoryComparison/twoStageCovariance/{iter}/9999/c2/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/twoStageRaremetal/{iter}/9999/c2/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/fullCovariance/{iter}/9999/c2/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/fullRaremetal/{iter}/9999/c2/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/timeOutputs/twoStage/{iter}/9999/c2/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("memoryComparison/timeOutputs/trad/{iter}/9999/c2/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
}
...
for(iter in iters){
  ...
  for(gene_curr in genes[[1]]){
    dir_to_make <- str_glue("memoryComparison/twoStageCovariance/{iter}/9999/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("memoryComparison/twoStageRaremetal/{iter}/9999/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("memoryComparison/fullCovariance/{iter}/9999/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("memoryComparison/fullRaremetal/{iter}/9999/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("memoryComparison/timeOutputs/twoStage/{iter}/9999/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("memoryComparison/timeOutputs/trad/{iter}/9999/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
...
for(iter in iters){
  for(i in 1:length(non_empty_gene)){
    ...
    altCovariancePath <- str_glue("memoryComparison/twoStageCovariance/{iter}/9999/c2/{gene}/")
    altRaremetalPath <- str_glue("memoryComparison/twoStageRaremetal/{iter}/9999/c2/{gene}/")
    outputFile <- str_glue("memoryComparison/timeOutputs/twoStage/{iter}/9999/c2/{gene}/time.txt")
    ...
    altCovariancePath <- str_glue("memoryComparison/fullCovariance/{iter}/9999/c2/{gene}/")
    altRaremetalPath <- str_glue("memoryComparison/fullRaremetal/{iter}/9999/c2/{gene}/")
    outputFile <- str_glue("memoryComparison/timeOutputs/trad/{iter}/9999/c2/{gene}/time.txt")
