...
for(gene_curr in genes[[1]]){
  dir_to_make <- str_glue("twoStageGroupFile/c2/{gene_curr}/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("twoStageCovariance/c2/{gene_curr}/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("twoStageRaremetal/c2/{gene_curr}/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("fullGroupFile/c2/{gene_curr}/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("fullCovariance/c2/{gene_curr}/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
  
  dir_to_make <- str_glue("fullRaremetal/c2/{gene_curr}/")
  if(!dir.exists(dir_to_make)){
    dir.create(dir_to_make)
  }
...
for(i in 1:length(non_empty_gene)){
  ...
  altCovariancePath <- str_glue("twoStageCovariance/c2/{gene}/")
  altRaremetalPath <- str_glue("twoStageRaremetal/c2/{gene}/")
  ...
  altCovariancePath <- str_glue("fullCovariance/c2/{gene}/")
  altRaremetalPath <- str_glue("fullRaremetal/c2/{gene}/")
...
df <- data.frame(gene = non_empty_gene,
                 trad_user = trad_user, 
                 trad_elapsed = trad_elapsed, 
                 two_stage_user = two_stage_user,
                 two_stage_elapsed = two_stage_elapsed)

write.table(df, file = "run1.res", quote = FALSE, row.names = FALSE)

################################################################################
#Now for the other four iterations
library("stringr")
library("externalCov")
...
for(iter in iters){
  print(iter)
  for(gene_curr in genes[[1]]){
    dir_to_make <- str_glue("twoStageGroupFile/{iter}/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("twoStageCovariance/{iter}/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("twoStageRaremetal/{iter}/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("fullGroupFile/{iter}/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("fullCovariance/{iter}/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
    
    dir_to_make <- str_glue("fullRaremetal/{iter}/c2/{gene_curr}/")
    if(!dir.exists(dir_to_make)){
      dir.create(dir_to_make)
    }
...
for(iter in iters){
  ...
  for(i in 1:length(non_empty_gene)){
    ...
    pLOF <- pLOF_narrowMissense <- pLOF_broadMissense <- FALSE
    altGroupFilePath <- NULL 
    altCovariancePath <- str_glue("twoStageCovariance/{iter}/c2/{gene}/")
    altRaremetalPath <- str_glue("twoStageRaremetal/{iter}/c2/{gene}/")
    altRaremetalName <- NULL 
    mafThreshold <- 0.01
    hwe <- 0.000001
    ...
    altCovariancePath <- str_glue("fullCovariance/{iter}/c2/{gene}/")
    altRaremetalPath <- str_glue("fullRaremetal/{iter}/c2/{gene}/")
...
#First read results from first run into the session where the remaining 4 runs were performed
df_run1 <- read.table("run1.res", header = TRUE)
...
write.table(df_run2, file = "run2.res", quote = FALSE, row.names = FALSE)
...
write.table(df_run3, file = "run3.res", quote = FALSE, row.names = FALSE)
...
write.table(df_run4, file = "run4.res", quote = FALSE, row.names = FALSE)
...
#Just do a table of the summaries for each 
summary(df_var$two_stage_elapsed_var)
summary(df_var$trad_elapsed_var)
