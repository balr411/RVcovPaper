...
p <- add_argument(p, "--allelefreq", help = "Allele frequency file in form CHROM,POS,REF,ALT,AC, AF")
...
p <- add_argument(p, "--study1", help = "Name of zipped RAREMETALWORKER study 1 score statistics")
...
file_name_curr <- argv$study1
study1_af <- fread(cmd = str_glue("zgrep -v ^## {file_name_curr}"), header = TRUE, data.table = FALSE)
vars <- study1_af[[2]]
vars_duplicated <- vars[duplicated(vars)]

allele_freq <- read.table(argv$allelefreq, header = FALSE)
names(allele_freq) <- c("CHROM", "POS", "REF", "ALT", "AF", "AC")
anno <- read.table(argv$anno, header = TRUE) 

#Delete multi-alleleic variants 
allele_freq <- allele_freq[!(allele_freq$POS %in% vars_duplicated),]

#Delete variants with AF > 0.01 and AF = 0
allele_freq <- allele_freq[allele_freq[,6] > 0 & allele_freq[,6] <= 0.01,]
...
allele_freq_ptv <- allele_freq[allele_freq[,6] <= 0.01,]

#Create normal group file containing only missense variants with MAF <0.01
allele_freq_narrowMissense <- allele_freq[allele_freq[,6] <= 0.01,]

#Create normal group file containing only non-synomynous variants with MAF < 0.01 
allele_freq_broadMissense <- allele_freq[allele_freq[,6] <= 0.01,]
...
if(length(mask_list[[1]]) > 1){
  ...
  group_table[1,3] <- group_table[1,3] + sum(allele_freq_ptv[idx_temp,5])
}
...
if(length(mask_list[[1]]) > 1){
  ...
  group_table[1,5] <- group_table[1,5] + sum(allele_freq_narrowMissense[idx_temp,5])
}
...
if(length(mask_list[[1]]) > 1){
  ...
  group_table[1,7] <- group_table[1,7] + sum(allele_freq_broadMissense[idx_temp,5])
}
