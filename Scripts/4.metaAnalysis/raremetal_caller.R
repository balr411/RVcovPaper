...
if(as.logical(argv$trueCov)){
  ...
  for(prefix in group_file_prefix){
    if(prefix %in% non_empty_group_file_prefix){
      ...
      burden_path <- paste("raremetalResults/burden", prefix, chr, test, traitname, gene, "", sep = "/")
      full_prefix <- paste0(burden_path, full_prefix)
      
      cm <- str_glue("/net/dumbo/home/welchr/projects/nullmetal/cmake-build-release/bin/raremetal --summaryFiles {summary_files} --covFiles {cov_file} --groupFile {group_file} --burden --maf 1 --hwe 0.000001 --prefix {full_prefix}")
      system(cm)
    }else{
...
if(as.logical(argv$estCov)){
  ...
  for(prefix in group_file_prefix){
    if(prefix %in% non_empty_group_file_prefix){
      ...
      burden_path <- paste("raremetalResults/burden", prefix, chr, test, ref, traitname, gene, "", sep = "/")
      full_prefix <- paste0(burden_path, full_prefix)
      cm <- str_glue("/net/dumbo/home/welchr/projects/nullmetal/cmake-build-release/bin/raremetal --summaryFiles {summary_files} --covFiles {cov_file} --groupFile {group_file} --burden --maf 1 --hwe 0.000001 --prefix {full_prefix}")
      system(cm)
    }else{
