...
p <- add_argument(p, "--pooledAF", help = "Path to pooled allele frequency file")
...
if(!is.null(dim(genotype_matrix_current))){ #This means there are either 0 variants or > 1 variant in the gene
  if(dim(genotype_matrix_current)[2] > 0){
    ...
    impCov <- output_cov_files
    cm <- str_glue("bgzip {impCov}")
    system(cm)
    
    imp0Cov_gzip <- paste0(output_cov_files, ".gz")
    cm <- str_glue("tabix -c \"#\" -s 1 -b 2 -e 2 {imp0Cov_gzip}")
    system(cm)
