...
p <- add_argument(p, "--PTV", help = "Include protein truncating variant group file? Options are T/F, name of file must be 
                  in format groupFilesNew/PTV/{chr}/{test}/{trait}/{chr}.{test}.{trait}.{gene}.PTV.group.file")
p <- add_argument(p, "--narrowMissense", help = "Include pLOF + missense(narrow) variant group file? Options are T/F, name of file must be 
                  in format groupFilesNew/MISSENSE/{chr}/{test}/{trait}/{chr}.{test}.{trait}.{gene}.MISSENSE.group.file")
p <- add_argument(p, "--broadMissense", help = "Include pLOF + missense(broad) variant group file? Options are T/F, name of file must be 
                  in format groupFilesNew/BROADMISSENSE/{chr}/{test}/{trait}/{chr}.{test}.{trait}.{gene}.BROADMISSENSE.group.file")
...
p <- add_argument(p, "--estCovFilesImp0UKBB", help = "Location of UKBB imp0 covariance files for raremetal", default = NULL)
p <- add_argument(p, "--estCovFilesImp0InPSYght", help = "Location of InPSYght imp0 covariance files for raremetal", default = NULL)
...
if(as.logical(argv$trueCov)){
  ...
  for(prefix in group_file_prefix){
    if(prefix %in% non_empty_group_file_prefix){
      ...
      burden_path <- paste("raremetalNew/burden", prefix, chr, test, traitname, gene, "", sep = "/")
      ...
      burden_path <- paste("raremetalNew/burden", prefix, chr, test, traitname, gene, "", sep = "/")
      file_burden <- paste(c(burden_path, paste(argv$chr, argv$testsize, argv$traitname, argv$gene, full_prefix, "meta.burden.results", sep = ".")), collapse = "")
      #file_SKAT <- paste(argv$chr, argv$testsize, argv$refsize, argv$traitname, argv$gene, full_prefix, "meta.SKAT_.results", sep = ".")
...
if(as.logical(argv$estCov)){
  for(prefix in group_file_prefix){
    if(prefix %in% non_empty_group_file_prefix){
      #UKBB reference panel
      cov_file_UKBB <- argv$estCovFilesImp0UKBB
      group_file_path <- paste("groupFilesNew", prefix, chr, test, traitname, "", sep = "/")
      group_file <- paste(c(group_file_path, paste(chr, test, traitname, gene, prefix, "group.file", sep = ".")), collapse = "")
      full_prefix <-  paste(chr, test, ref, traitname, gene, paste0("IMPUTED0_UKBB_", prefix), sep = ".")
      burden_path <- paste("raremetalNew/burden", prefix, chr, test, ref, traitname, gene, "", sep = "/")
      full_prefix <- paste0(burden_path, full_prefix)
      cm <- str_glue("/net/snowwhite/home/welchr/projects/raremetal/build/release/bin/raremetal --summaryFiles {summary_files} --covFiles {cov_file_UKBB} --groupFile {group_file} --burden --maf 1 --hwe 0.000001 --prefix {full_prefix}")
      system(cm)
      
      #InPSYght reference panel
      cov_file_InPSYght <- argv$estCovFilesImp0InPSYght
      group_file_path <- paste("groupFilesNew", prefix, chr, test, traitname, "", sep = "/")
      group_file <- paste(c(group_file_path, paste(chr, test, traitname, gene, prefix, "group.file", sep = ".")), collapse = "")
      full_prefix <-  paste(chr, test, ref, traitname, gene, paste0("IMPUTED0_InPSYght_", prefix), sep = ".")
      burden_path <- paste("raremetalNew/burden", prefix, chr, test, ref, traitname, gene, "", sep = "/")
      full_prefix <- paste0(burden_path, full_prefix)
      cm <- str_glue("/net/snowwhite/home/welchr/projects/raremetal/build/release/bin/raremetal --summaryFiles {summary_files} --covFiles {cov_file_InPSYght} --groupFile {group_file} --burden --maf 1 --hwe 0.000001 --prefix {full_prefix}")
      system(cm)
    }else{
      full_prefix <- paste0("IMPUTED0_UKBB_", prefix)
      burden_path <- paste("raremetalNew/burden", prefix, chr, test, ref, traitname, gene, "", sep = "/")
      ...
      full_prefix <- paste0("IMPUTED0_InPSYght_", prefix)
      burden_path <- paste("raremetalNew/burden", prefix, chr, test, ref, traitname, gene, "", sep = "/")
      file_burden <- paste(c(burden_path, paste(argv$chr, argv$testsize, argv$refsize, argv$traitname, argv$gene, full_prefix, "meta.burden.results", sep = ".")), collapse = "")
      file.create(file_burden)
    }
